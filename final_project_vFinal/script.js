...
// answer check
  ...
  document.getElementById("check").addEventListener("click", () => {
    ...
    if (userAnswer === correctAnswer) {
        ...
        feedback.style.background = "#70cf0b"; //green background
      ...
      if (currentLevel < levels.length) {
        setTimeout(loadLevel, 1000); //1s to load new level
      } else {
        feedback.style.display = "block";
        feedback.style.background = "#348edd"; //light blue background
        feedback.textContent = "🎈🏆🎉 You are a genius ! See you in the next session... 🎊🥳🌟";
        triggerFinalCelebration();
        clearInterval(timerInterval);
        setTimeout(() => {
        location.reload();
      }, 3000); // 3 seconds to reload the game
      }
    } else {
      feedback.textContent = "Oops! wrong answer...";
      feedback.style.display = "block"; // s'assurer qu'on l'affiche d'abord
      feedback.style.background = "red"; //red background
